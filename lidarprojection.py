...
def main(args):
    ...
    gt_semantic_folder = os.path.join(args.muses_root, 'gt_semantic', 'train', 'clear', 'day')
    ...
    for gt_image_name, metadata in meta_data.items():
        ...
        gt_image_name = gt_image_name + '_gt_labelColor.png'
        ...
        # inspect GT image and projected image shape matched or not 
        ...
        # Assign GT annotation to LiDAR point
        lidar_annotations = []
        for point in lidar_points_2d:
            x, y = int(point[0]), int(point[1])
            if 0 <= x < gt_image.shape[1] and 0 <= y < gt_image.shape[0]:
                label = int(gt_image[y, x])  # Convert label to a JSON serializable type
                lidar_annotations.append(label)
            else:
                lidar_annotations.append(None)

        # Save lidar and related GT annotation to file 
        output_data = {
            'lidar_points': lidar_points_2d.tolist(),
            'annotations': lidar_annotations
        }
        output_filename = os.path.join(output_folder, f'annotations_{gt_image_name}.json')
        with open(output_filename, 'w') as f:
            json.dump(output_data, f, indent=4)
        print(f"Saved annotations for {gt_image_name} as {output_filename}.")
